...
await bot.api.setWebhook(new URL("api/update", `https://${VERCEL_URL}`));

await Promise.all(Object.entries(Utils.getStringByLocales("bot-name")).map(([locale, string]) => {
    return bot.api.setMyName(string, {language_code: Utils.filterLocale(locale)});
}));

await Promise.all(Object.entries(Utils.getStringByLocales("bot-description")).map(([locale, string]) => {
    return bot.api.setMyShortDescription(string, {language_code: Utils.filterLocale(locale)});
}));
